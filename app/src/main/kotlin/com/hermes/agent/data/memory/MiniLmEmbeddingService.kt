 ...
 * "AI Models/embeddings/all-MiniLM-L6-v2" folder — the same files the standalone
 * Octo Jotter / Hermes model manager download, so a model already on the device
 ...
 *
 * ONNX Runtime is already a dependency here (Butler's TTS), so this adds no new
 * native library to the APK. Inference is serialized behind a mutex — one
 * OrtSession, one call at a time.
 */
@Singleton
class MiniLmEmbeddingService @Inject constructor(
    ...
    private fun runSession(enc: WordPieceTokenizer.Encoding): FloatArray {
        ...
        try {
            ...
            // ORT's Result is AutoCloseable (not Closeable), so close it manually
            // rather than via `use` — mirrors the Butler TtsEngine pattern.
